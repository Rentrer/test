...
class MyThread(threading.Thread):
    def __init__(self, url_queue):
        ...
        # 连接Mysql数据库
        self.cnn = pymysql.connect(host='127.0.0.1', user='root', password='12345678', port=3306, database='news_with_keyword',
                                   charset='utf8')
...
class NewsCrawler:
    ...
    def save(self, item):
        """将新闻保存到数据库"""
        sql = """INSERT INTO guanchazhe(title, author, publish_time, content, url, key_word) 
                VALUES(%s, %s, %s, %s, %s, %s)"""
        try:
            self.cursor.execute(sql, (
                item['title'], 
